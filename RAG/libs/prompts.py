...
5. 严格按字段为question、answer和reason的JSON对象格式返回，不要输出```json```，也不要输出其他无关内容。
...
4. 严格按字段为score和reason的JSON对象格式返回，不要输出```json```，也不要输出其他无关内容。
...
3. 严格按字段为score的JSON对象格式返回，不要输出```json```，也不要输出其他无关内容。
